...
class Cron::InvoicesController < AuthenticatedNonResourceController
  ...
  def personMemberInvoices(year)
    @person_members = PersonMember.includes(%i[tariff
                                               member]).joins("LEFT JOIN member_account_bookings mb ON person_members.member_id=mb.member_id AND mb.booking_type='B' and YEAR(mb.booking_date) = YEAR(NOW())").where('mb.id IS NULL').order('members.mglnr')
    ...
    @person_members.each do |person|
      ...
      next unless person.is_direct_debit?
