...
def consider_regular_tickets(invoice, count)
    ...
    item = invoice.consider_item(count, prices["ticket_entry"]+prices["ticket_food"]+prices["ticket_drink"], I18n.t("event_card.fest"), vat: 7)
...
def consider_reduced_tickets(invoice, count)
    ...
    item = invoice.consider_item(count, prices["ticket_entry_red"]+prices["ticket_food"]+prices["ticket_drink"], I18n.t("event_card.fest_erm"), vat: 7)
    if item.present?
      item.item_taxes.clear()
      item.add_split_tax(7,prices["ticket_food"], I18n.t("event_card.food_part"))
      item.add_split_tax(19, prices["ticket_drink"], I18n.t("event_card.drink_part"))
